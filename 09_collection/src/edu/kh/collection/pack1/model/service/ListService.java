...
public class ListService {
	...
	public void method1() {
		...
		list.add("아무거나");
		list.add(new Object());
		list.add(123123); // -> int가 Integer로 변환(Wrapper Class)로 변환
											//  -> 컴파일러가 자동으로 변환
		
		list.add(3.123213); // 크기 3초과 했을 때 오류 발생 확인
		list.add(false);		// -> 오류 X 왜? 크기가 자동으로 늘어나서
		
		
		System.out.println(list);
		
		// 2. E get(int index) L List에서 index 번째 요소 반환 받기
		...
		for(int i = 0; i < list.size(); i++) {
			...
			// List의 i번쨰 요소가 int(-> Integer 변환)인 경
			if(list.get(i) instanceof Integer) {
				System.out.println("****Integer 자료형 입니다****");
				continue;
			}
			
			// List의 i번쨰 요소가 double(-> Double 변환)인 경
